<?php
include $_SERVER['DOCUMENT_ROOT'] . '/includes/dashboard/purchases/purchases.php';
?>
...
<body class="bg-gray-900 text-gray-100 min-h-screen flex flex-col">
    ...
    <div class="flex flex-1">
        <main class="flex-1 p-8 md:ml-64">
            <div class="flex justify-between items-center mb-4">
                ...
                <button id="open-create-btn" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded">Create New Purchase</button>
            </div>

            <?php if (!empty($_SESSION['errorMessage'])): ?>
            <script>
                alert("<?= addslashes($_SESSION['errorMessage']) ?>");
            </script>
            ...
            <div class="bg-gray-800 p-6 rounded shadow-lg mb-8">
                ...
                <div class="overflow-x-auto">
                    <table id="purchases-table" class="w-full table-auto border-collapse">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr class="bg-gray-800 hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2 border"><?= htmlspecialchars($purchase['user_name']) ?></td>
                                ...
                                <td class="px-4 py-2 border space-x-2">
                                    <button
                                        ...
                                        data-user_id="<?= htmlspecialchars($purchase['user_id']) ?>"
                                        data-product_id="<?= htmlspecialchars($purchase['product_id']) ?>"
                                        data-suspended="<?= htmlspecialchars($purchase['suspended']) ?>"
                                    >Edit</button>
                            ...
                            <tr>
                                <td colspan="8" class="px-4 py-2 border text-center">No purchases found.</td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    <div id="edit-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
            <h3 class="text-xl font-semibold mb-4 text-gray-100">Edit Purchase</h3>
            <form method="POST" id="edit-form" class="space-y-4">
                <input type="hidden" name="action" value="edit" />
                <input type="hidden" name="id" id="edit-id" />
                <div>
                    <label for="edit-user_id" class="block mb-1">User</label>
                    <select id="edit-user_id" name="user_id" required class="w-full px-3 py-2 bg-gray-700 rounded text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                    <label for="edit-product_id" class="block mb-1">Product</label>
                    <select id="edit-product_id" name="product_id" required class="w-full px-3 py-2 bg-gray-700 rounded text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                    <label for="edit-suspended" class="block mb-1">
                        <input type="checkbox" id="edit-suspended" name="suspended" class="mr-2 leading-tight">Suspended
                    </label>
                </div>
    ...
    <div id="create-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
            <h3 class="text-xl font-semibold mb-4 text-gray-100">Create New Purchase</h3>
            <form method="POST" id="create-form" class="space-y-4">
                <input type="hidden" name="action" value="create" />
                <div>
                    <label for="create-user_id" class="block mb-1">User</label>
                    <select id="create-user_id" name="user_id" required class="w-full px-3 py-2 bg-gray-700 rounded text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <?php foreach ($users as $user): ?>
                            <option value="<?= $user['id'] ?>"><?= htmlspecialchars($user['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    <label for="create-product_id" class="block mb-1">Product</label>
                    <select id="create-product_id" name="product_id" required class="w-full px-3 py-2 bg-gray-700 rounded text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <?php foreach ($products as $product): ?>
                            <option value="<?= $product['id'] ?>"><?= htmlspecialchars($product['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    <label for="create-suspended" class="block mb-1">
                        <input type="checkbox" id="create-suspended" name="suspended" class="mr-2 leading-tight">Suspended
                    </label>
                </div>
                <div class="flex justify-end space-x-2">
                    <button type="button" id="create-cancel" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded">Cancel</button>
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Create Purchase</button>
                </div>
            </form>
        </div>
    </div>

    <script src="/includes/dashboard/purchases/modals.js"> </script>
</body>
</html>