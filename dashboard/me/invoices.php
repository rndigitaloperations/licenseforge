<?php include $_SERVER['DOCUMENT_ROOT'] . '/includes/dashboard/me/invoices.php'; ?>
                ...
                <div class="overflow-x-auto">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-gray-800 hover:bg-gray-700">
                            <td class="px-4 py-2 border"><?= htmlspecialchars($invoice['id']) ?></td>
                            <td class="px-4 py-2 border"><?= htmlspecialchars($invoice['product_name']) ?></td>
                            <td class="px-4 py-2 border"><?= htmlspecialchars($invoice['status']) ?></td>
                            <td class="px-4 py-2 border"><?= htmlspecialchars($invoice['created_at']) ?></td>
                            <td class="px-4 py-2 border">
                                <?php if ($invoice['status'] === 'unpaid'): ?>
                                    <a href="/invoices/<?= (int)$invoice['id'] ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded inline-block">View</a>
                                    <a href="/invoices/pay/<?= (int)$invoice['id'] ?>" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded inline-block">Pay</a>
                                <?php else: ?>
                                    <a href="/invoices/<?= (int)$invoice['id'] ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded inline-block">View</a>
                                <?php endif; ?>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-2 border text-center">No invoices found.</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
